...
#lambdas=[2,5,10,15,20,25]
# Define the set of orientations for the Gaussian envelope.
dTheta       =  np.pi/6   # \\ the step size
orientations = np.arange(0, np.pi/2+np.pi/3, dTheta)        
...
for gaborFilter in gaborFilterBank:
    ...
    real_out =  convolve2d(img_gray,gaborFilter['filterPairs'][:,:,0],mode='same')# \\TODO: filter the grayscale input with real part of the Gabor
    imag_out =  convolve2d(img_gray,gaborFilter['filterPairs'][:,:,1],mode='same') # \\TODO: filter the grayscale input with imaginary part of the Gabor
...
if smoothingFlag:
    for i in range(len(featureMags)):
        features[:,:,i]=cv2.GaussianBlur(featureMags[i],(5,5),1)
    # \\TODO:
    #FOR_LOOP
...
plt.imshow(Aseg2, 'jet',  interpolation='none', alpha=1)
plt.axis("off") 
